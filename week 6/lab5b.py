"""
    ...
    For this version of Lab 5, you will be creating a script that prompts the user for a password using the built-in getpass function. This lab (much like the prompt() function that we created previously) is intended to be imported as a module into future labs.
    ...
    With the exception of strong_password(), each has_ function noted above will perform a single check (for uppercase, lowercase, number, etc.) and return True or False. 
